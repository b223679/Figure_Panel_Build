...
public class FigureConfiguration {
  ...
  public InsetCell insetCell(int conditionIndex, int displayIndex) {
    ...
    if (cell == null) cell = new InsetCell();
    ConditionConfig condition = conditions.get(conditionIndex);
    if (!condition.shareInsetRoi) return cell;
    InsetCell effective = new InsetCell();
    effective.enabled = cell.enabled;
    effective.x = condition.sharedRoiX; effective.y = condition.sharedRoiY;
    return effective;
  }
