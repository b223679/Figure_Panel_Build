...
public final class FreeBuildDialog extends JFrame {
  ...
  public FreeBuildDialog(int rows, int columns) {
    ...
    JToggleButton mode = new JToggleButton("Free mode ON", true); top.add(mode);
  ...
  private void chooseImage() {
    ...
    File file = chooseFile(false,"tif"); if (file == null) return;
    final int slot = selected;
    InputImageManager candidate = inputs.copy();
    busy = true; status.setText("Loading " + file.getName());
    new SwingWorker<InputImageManager.Source,Void>() {
      protected InputImageManager.Source doInBackground() { return candidate.load(file); }
      protected void done() {
