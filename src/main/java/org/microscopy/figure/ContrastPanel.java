...
public class ContrastPanel extends JPanel {
  ...
  private final JCheckBox grayscale = new JCheckBox("Grayscale");
  ...
  public ContrastPanel(FigureConfiguration c, InputImageManager i, Runnable changed, boolean compact) {
    ...
    top.add(lut);
    grayscale.setName("grayscaleOverride");
    grayscale.setToolTipText(
        "View this channel in grayscale; its real LUT is kept and still used inside any Merge.");
    top.add(grayscale);
    invert.setName("invertGray");
    invert.setToolTipText("Invert the grayscale display only; source pixels remain unchanged.");
    top.add(invert);
    ...
    grayscale.addActionListener(e -> {
      if (updating || selector.getSelectedIndex() < 0) return;
      current().grayscale = grayscale.isSelected(); timer.restart(); changed.run();
    });
    ...
    if (!c.channels.isEmpty()) select();
      ...
      grayscale.setEnabled(false); invert.setEnabled(false);
  ...
  private void sync() {
    ...
    try {
      ...
      grayscale.setSelected(cc.grayscale);
