...
public class FigurePanelBuilderDialog extends JFrame {
  ...
  FigurePanelBuilderDialog(boolean selectOnStartup) {
    ...
    menu.add(toolbar, BorderLayout.CENTER); menu.add(freeMode, BorderLayout.EAST);
  ...
  private void addSource(InputImageManager.Source source) {
    ...
    if (config.channels.isEmpty()) {
      for (int c = 1; c <= source.channels; c++) {
        ChannelConfig cc = new ChannelConfig(c, source.channelNames.get(c - 1), ChannelConfig.defaultLut(c));
  ...
  private void removeEntry(boolean channel, int index) {
    ...
    if (channel) {
      ...
      config.conditions.remove(index);
      // Removing the last condition must clear channel/display definitions too: otherwise they
      // stay stale (sized/indexed for the old images) and block adding a differently-sized or
      // differently-channeled image next, until the plugin is restarted.
      if (config.conditions.isEmpty()) {
        config.channels.clear();
        config.displayChannels.clear();
        appearanceInitialized = false;
        selectedDisplay = -1;
      }
    }
    refresh();
  ...
  private void rebuildDock() {
    ...
    if (config.channels.isEmpty() || config.conditions.isEmpty()) {
      ...
      dockContrast.setPreferredSize(new Dimension(530, 172));
