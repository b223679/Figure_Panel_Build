...
public class FreeBuildUiValidation {
  ...
  static void add(int slot,int...channels)throws Exception {
    edt(()->{
      choose(new File("test-data/Control.tif"));respond(w->title(w).equals("Select Channels"),w->{all(w,JList.class).get(0).setSelectedIndices(channels);pane(w).setValue(JOptionPane.OK_OPTION);});
      click(slot);
