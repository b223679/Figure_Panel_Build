...
class UiModelTest {
  ...
  void groupedAppearanceControlsUpdateIndependentSettings() throws Exception {
    SwingUtilities.invokeAndWait(
        () -> {
          ...
          // White background auto-contrasts both row and column label color to Black.
          assertFalse(c.labels.isWhiteText(true));
          assertFalse(c.labels.isWhiteText(false));
  ...
  @Test
  void backgroundChoiceAutoContrastsLabelColorButLeavesTransparentManual() throws Exception {
    SwingUtilities.invokeAndWait(
        () -> {
          FigureConfiguration c = new FigureConfiguration();
          AppearancePanel panel = new AppearancePanel(c, () -> {});
          named(panel, JToggleButton.class, "backgroundSecond").doClick();
          assertTrue(c.labels.whiteBackground == false && c.labels.transparentBackground == false);
          assertTrue(c.labels.isWhiteText(true), "Black background should auto-select white row labels");
          assertTrue(c.labels.isWhiteText(false), "Black background should auto-select white column labels");
          assertTrue(named(panel, JToggleButton.class, "rowColorSecond").isSelected());
          assertTrue(named(panel, JToggleButton.class, "columnColorSecond").isSelected());
          named(panel, JToggleButton.class, "backgroundThird").doClick();
          assertTrue(c.labels.transparentBackground);
          // Transparent background leaves the prior manual/auto label color choice untouched.
          assertTrue(c.labels.isWhiteText(true));
