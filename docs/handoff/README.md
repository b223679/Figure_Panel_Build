...
> 以下は過去のセッションの記録です。「未コミット」「現在30件」、旧機能の仕様、配布済みという記述は当時の状態であり、現状を示しません。現在の操作・配布手順は [README](../../README.md)、検証結果は [VALIDATION.md](../development/VALIDATION.md)、開発時のコミット方針は [AGENTS.md](../../AGENTS.md) を参照してください。過去の画面画像・生成画像は整理時に削除済みで、`artifacts/` は Git 管理外です。

このフォルダは、直近のセッションで行った変更内容と、プロジェクトを引き継ぐ人（未来の自分を含む）が最短で状況を把握できるようにするための資料です。詳細な機能仕様は `../development/仕様書.md`、導入・操作方法は `../../README.md`、検証記録は `../development/VALIDATION.md`、開発ルールは `../../AGENTS.md` を参照してください。本資料はそれらを読む前に「今どうなっているか」を把握するための入口です。
