...
## 2026-09-14 起動位置とネイティブ最大化

- 通常・Free buildとも自動最大化を廃止。対象モニターのタスクバーを除いた領域に、横中央・縦は余白の上側1/3の位置で配置する。
- 初期サイズは従来サイズを上限に、有効幅95%・高さ97%以内へ縮小。小さい画面では最小サイズも初期サイズ以下に制限する。
- 独自の最大化範囲指定を削除し、Windowsにタスクバー・DPI・タイトルバーの処理を任せる。設定JSONと画像生成・出力処理は変更なし。
- `build.ps1` 増分verify成功：49 tests、失敗0・エラー0。
- 独立JVMの `WindowPlacementUiValidation` 成功。両モードで通常状態、使用可能領域内、中央・やや上、標準タイトルバー、手動最大化と復帰を確認。
- 実画面のAWT論理座標：有効領域1440×936、初期位置(36,12)、サイズ1368×900。通常・最大化のキャプチャで下部操作ボタンと右上ボタン一組を目視確認。画像は `artifacts/window-placement/`（Git対象外）。検証時のみテストウィンドウを最前面に設定。
- ユーザーのFijiプロセス・開いている画像・入力ファイルは変更していない。別のDPI/マルチモニター配置は未実機検証。
