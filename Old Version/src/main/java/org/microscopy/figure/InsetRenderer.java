...
public class InsetRenderer {
  ...
  public boolean applies(InsetCell cell, InputImageManager.Source source, InsetConfig config) {
    ...
    if (roiX < 0 || roiY < 0 || roiX + config.roiWidth > source.width || roiY + config.roiHeight > source.height)
      return false;
    return config.boxWidth() + 2L * config.marginX <= source.width
        && config.boxHeight() + 2L * config.marginY <= source.height;
  }
  ...
  public static Point boxOrigin(InsetConfig config, int width, int height) {
    ...
    boolean bottom =
        config.position == InsetConfig.Position.BOTTOM_LEFT
            || config.position == InsetConfig.Position.BOTTOM_RIGHT;
    int x = right ? width - config.marginX - config.boxWidth() : config.marginX;
    int y = bottom ? height - config.marginY - config.boxHeight() : config.marginY;
  ...
  public void draw(
    ...
    if (insetConfig.boxWidth() + 2L * insetConfig.marginX > source.width
        || insetConfig.boxHeight() + 2L * insetConfig.marginY > source.height)
      throw new IllegalArgumentException("Inset box does not fit cell: " + source.title);
    BufferedImage crop =
        new ImageRenderer()
            .render(source, display, config, roiX, roiY, roiW, roiH, insetConfig.boxWidth(), insetConfig.boxHeight());
    ...
    try {
      ...
      Shape box = insetConfig.sameAsRoi && insetConfig.shape == InsetConfig.Shape.CIRCLE
          ? new Ellipse2D.Double(x, y, insetConfig.boxWidth(), insetConfig.boxHeight())
          : new Rectangle2D.Double(x, y, insetConfig.boxWidth(), insetConfig.boxHeight());
      Shape clip = g.getClip();
      g.clip(box);
      g.drawImage(crop, x, y, insetConfig.boxWidth(), insetConfig.boxHeight(), null);
      g.setClip(clip);
      if (insetConfig.insetStrokeWidth > 0) {
        g.setStroke(new BasicStroke(insetConfig.insetStrokeWidth));
        g.draw(box);
      }
      if (insetConfig.roiStrokeWidth == 0) return;
