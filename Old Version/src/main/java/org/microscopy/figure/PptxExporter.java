...
public final class PptxExporter {
    ...
    try {
      try(ZipOutputStream out=new ZipOutputStream(Files.newOutputStream(temp))) {
        ...
        for(int ci=0;ci<c.conditions.size();ci++) for(int di=0;di<c.displayChannels.size();di++) {
          ...
          inset(out, source, c.displayChannels.get(di), c, insetCell, x, y, 1, 1);
  ...
  public void saveFree(File destination, FreeBuildConfiguration c, InputImageManager inputs) throws IOException {
    OutputSafety.checkDestination(destination, inputs);
    java.util.List<FreeBuildConfiguration.Item> items = c.scene(inputs, 1, true);
      ...
      try (ZipOutputStream out = new ZipOutputStream(Files.newOutputStream(temp))) {
        ...
        for (FreeBuildConfiguration.Item item : items) {
          ...
          if (item.image == null) {
            ...
            if (item.insetConfiguration != null) {
              FigureConfiguration config = item.insetConfiguration;
              inset(out, item.insetSource, config.displayChannels.get(0), config, config.insetCell(0,0),
                  b.x, b.y, (double)b.width/item.insetSource.width, (double)b.height/item.insetSource.height);
            }
          }
        }
  ...
  private String outline(double width, Color color) {
    return width <= 0 ? "<a:ln><a:noFill/></a:ln>"
        : "<a:ln w=\"" + emu(width) + "\"><a:solidFill><a:srgbClr val=\"" + hex(color) + "\"/></a:solidFill></a:ln>";
  }

  /** Native crop picture and ROI outline: neither is baked into the main panel image. */
  private void inset(ZipOutputStream out, InputImageManager.Source source, DisplayChannel display,
      FigureConfiguration config, InsetCell cell, double ox, double oy, double sx, double sy) throws IOException {
    InsetConfig i = config.inset;
    if (!new InsetRenderer().applies(cell, source, i)) return;
    int rx = cell.roiX(source,i), ry = cell.roiY(source,i);
    BufferedImage crop = new ImageRenderer().render(source,display,config,rx,ry,i.roiWidth,i.roiHeight,i.boxWidth(),i.boxHeight());
    String rel = "inset" + (++id), media = rel + ".png";
    ByteArrayOutputStream png = new ByteArrayOutputStream(); ImageIO.write(crop,"png",png);
    entry(out,"ppt/media/"+media,png.toByteArray());
    relationships.append("<Relationship Id=\"").append(rel).append("\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/image\" Target=\"../media/").append(media).append("\"/>");
    Point origin = InsetRenderer.boxOrigin(i,source.width,source.height);
    String geometry = i.sameAsRoi && i.shape == InsetConfig.Shape.CIRCLE ? "ellipse" : "rect";
    shapes.append("<p:pic><p:nvPicPr><p:cNvPr id=\"").append(id).append("\" name=\"Inset — movable crop\"/>")
        .append("<p:cNvPicPr><a:picLocks noChangeAspect=\"1\"/></p:cNvPicPr><p:nvPr/></p:nvPicPr><p:blipFill><a:blip r:embed=\"")
        .append(rel).append("\"/><a:stretch><a:fillRect/></a:stretch></p:blipFill><p:spPr>")
        .append(transform(ox+origin.x*sx,oy+origin.y*sy,i.boxWidth()*sx,i.boxHeight()*sy,0))
        .append("<a:prstGeom prst=\"").append(geometry).append("\"><a:avLst/></a:prstGeom>")
        .append(outline(i.insetStrokeWidth*Math.min(sx,sy),i.color.awt)).append("</p:spPr></p:pic>");
    if (i.roiStrokeWidth > 0) shapes.append("<p:sp><p:nvSpPr><p:cNvPr id=\"").append(++id).append("\" name=\"Inset ROI outline\"/>")
        .append("<p:cNvSpPr/><p:nvPr/></p:nvSpPr><p:spPr>")
        .append(transform(ox+rx*sx,oy+ry*sy,i.roiWidth*sx,i.roiHeight*sy,0))
        .append("<a:prstGeom prst=\"").append(i.shape == InsetConfig.Shape.CIRCLE ? "ellipse" : "rect")
        .append("\"><a:avLst/></a:prstGeom><a:noFill/>").append(outline(i.roiStrokeWidth*Math.min(sx,sy),i.color.awt))
        .append("</p:spPr></p:sp>");
  }
