...
public class AppearancePanel extends JPanel implements Scrollable {
  ...
  public AppearancePanel(FigureConfiguration c, Runnable changed) {
    ...
    // Filled once the row/column label sections below create their Color toggle pairs; referenced
    // (not reassigned) by the background listener so label color can auto-follow the background.
    JToggleButton[] rowColorButtons = new JToggleButton[2];
    JToggleButton[] columnColorButtons = new JToggleButton[2];
    ...
    for (int i = 0; i < backgroundNames.length; i++) {
      ...
      b.addActionListener(e -> {
        l.transparentBackground = option == 2; l.whiteBackground = option == 0; changed.run();
        if (option == 2) return; // Transparent: label color stays a manual choice.
        // Auto-contrast: white background -> black labels, black background -> white labels.
        boolean whiteText = option == 1;
        l.rowWhiteText = whiteText; l.columnWhiteText = whiteText;
        if (rowColorButtons[0] != null) rowColorButtons[whiteText ? 1 : 0].setSelected(true);
        if (columnColorButtons[0] != null) columnColorButtons[whiteText ? 1 : 0].setSelected(true);
      });
    }
    ...
    toggle(row, "Color", "Black", "White", l.isWhiteText(true), v -> l.rowWhiteText = v, "rowColor", rowColorButtons);
    ...
    toggle(column, "Color", "Black", "White", l.isWhiteText(false), v -> l.columnWhiteText = v, "columnColor", columnColorButtons);
  ...
  private void toggle(JPanel p, String text, String first, String second, boolean secondSelected, Consumer<Boolean> set, String name) {
    toggle(p, text, first, second, secondSelected, set, name, null);
  }
  private void toggle(JPanel p, String text, String first, String second, boolean secondSelected, Consumer<Boolean> set, String name, JToggleButton[] out) {
