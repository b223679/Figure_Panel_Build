...
public final class FreeBuildConfiguration {
  ...
  public static FigureConfiguration image(InputImageManager.Source s, Integer[] ids, InputImageManager inputs) {
    ...
    for (int id : ids) {
      ...
      ChannelConfig.Lut lut = s.channels == 1 ? ChannelConfig.Lut.Grayscale : ChannelConfig.defaultLut(id);
  ...
  public static final class Item {
    ...
    public int rotation;
    public FigureConfiguration insetConfiguration;
    public InputImageManager.Source insetSource;
  ...
  public List<Item> scene(InputImageManager inputs, double scale) {
    return scene(inputs, scale, false);
  }

  public List<Item> scene(InputImageManager inputs, double scale, boolean separateInsets) {
    validate(inputs);
    ...
    for (int n = 0; n < panels.size(); n++) {
      ...
      if (p.showName) items.add(new Item(null, source.title, b, 0));
        ...
        FigureConfiguration rendered = p.image;
        if (separateInsets) {
          rendered = new SettingsSerializer().copy(p.image);
          rendered.conditions.get(0).insets.clear();
        }
        BufferedImage image = new PanelLayoutEngine().render(rendered, inputs, Math.min(1, fit * scale));
        Item item = new Item(image, null, new Rectangle(b.x + (b.width-w)/2, b.y + (b.height-h)/2, w, h), 0);
        if (separateInsets) { item.insetConfiguration = p.image; item.insetSource = source; }
        items.add(item);
      }
