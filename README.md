...
2. Fiji で既に開いている画像を Ctrl / Shift クリックで選択、または `Open TIFF / LIF files...` でファイルを選択します（ドラッグ＆ドロップも可）。
...
- **通常モードのZスタック**：Zが2枚以上の場合、確認後にChannelごとのMAX Projectionを作成し、元ファイルと同じフォルダに`元の名前_MAX_一意な番号.tif`として保存して読み込みます。開いているZスタックは元ファイルの保存先が必要です。
- **通常モードのLeica LIF**：FijiのBio-Formatsを使用して全シリーズを読み、元LIFと同じフォルダに新しい`元の名前_TIFF_一意な番号`フォルダを作成します。各シリーズを個別のTIFFに保存し、Zが2枚以上なら確認後にMAX Projectionを保存して読み込みます。既存ファイルは上書きしません。
- T=1のグレースケール／複数Channel画像が対象です。T>1、RGB、およびFigure内で異なる画像サイズ・Channel数は従来どおり非対応です。サイズ等が異なるLIFでも変換済みTIFFは保存フォルダに残ります。Free modeの読み込み仕様は変更していません。
- **通常モードの終了**：右上の×から閉じると現在の設定を保存するか確認します。Yesで保存先を選び、保存成功後に終了します。Noで保存せず終了、Cancel・保存先選択の取消・保存失敗ではウィンドウを残します。
